...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setActivity(leads.slice(0,6).map(l => ({
        ...
        text: l.status==='booked'?`${l.name} ${tr.bookedBadge} — ${l.car}`:l.status==='sent'?`AI email → ${l.name} — ${l.car}`:`${l.name} — ${l.car}`,
  ...
  const funnelLabels = [tr.sent, tr.opened, tr.clicked, tr.replied, tr.booked, tr.sale]
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',gap:10,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'10px 16px',marginBottom:16}}>
        ...
        <div style={{fontSize:12,color:'var(--text2)'}}>
          <span style={{fontWeight:600,color:'var(--text)'}}>{tr.aiSendsAutomatically}</span>
          {' — '}{tr.aiStatusDesc}
        </div>
        <a href="/integrations" style={{marginLeft:'auto',fontSize:11,color:'var(--gold)',textDecoration:'none',flexShrink:0}}>{tr.adjustSettings}</a>
      </div>

      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
        {[
          {cls:'gold',label:tr.coldLeads,val:stats.cold.toString(),color:'var(--gold)',sub:tr.notContacted,trend:tr.readyForReactivation,tc:'var(--text3)'},
          {cls:'green',label:tr.emailsSent,val:stats.sent.toString(),color:'var(--green)',sub:tr.viaAiCampaigns,trend:tr.aiGeneratedEmails,tc:'var(--text2)'},
          {cls:'blue',label:tr.allLeadsKpi,val:stats.total.toString(),color:'var(--blue)',sub:tr.inDatabase,trend:tr.importedAndReady,tc:'var(--text2)'},
          {cls:'amber',label:tr.bookings,val:stats.booked.toString(),color:'var(--amber)',sub:tr.trialRides,trend:tr.viaAiOutreach,tc:'var(--text2)'},
        ].map(k=>(
      ...
      <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:14}}>
        <div>
          <div className="panel" style={{marginBottom:14}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
              <div className="font-head" style={{fontSize:13,fontWeight:600}}>{tr.aiActivity}</div>
              <div className="pill pill-green" style={{fontSize:10}}><span className="plan-dot"></span>{tr.autoRunning}</div>
            </div>
            {activity.length === 0 ? (
              <div style={{textAlign:'center',padding:'30px 0',color:'var(--text3)'}}>
                <div style={{fontSize:24,marginBottom:8}}>📭</div>
                <div style={{fontSize:13}}>{tr.noActivity}</div>
              </div>
            ) : activity.map((a,i)=>(
          ...
          <div className="panel">
            <div className="font-head" style={{fontSize:13,fontWeight:600,marginBottom:14}}>{tr.emailsSentChart}</div>
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:5}}>
              <span style={{fontSize:10,color:'var(--text3)'}}>{tr.daysAgo}</span>
              <span style={{fontSize:10,color:'var(--text3)'}}>{tr.today}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="panel" style={{marginBottom:14}}>
            <div className="font-head" style={{fontSize:13,fontWeight:600,marginBottom:14}}>{tr.conversionFunnel}</div>
            ...
            <div style={{marginTop:16,paddingTop:14,borderTop:'1px solid var(--border)'}}>
              <div style={{fontSize:10,color:'var(--text3)',marginBottom:4,textTransform:'uppercase',letterSpacing:'1px',fontWeight:600}}>{tr.estimatedRevenue}</div>
              <div style={{fontSize:11,color:'var(--text2)',marginBottom:6}}>{stats.booked} × {avgPrice.toLocaleString('da')} {symbol} × 40%</div>
          ...
          <div className="panel">
            <div className="font-head" style={{fontSize:13,fontWeight:600,marginBottom:14}}>Quick actions</div>
            <a href="/leads" style={{display:'block',marginBottom:7}}><button className="btn btn-ghost" style={{width:'100%',justifyContent:'center'}}>{tr.reviewLeads}</button></a>
            <a href="/import" style={{display:'block'}}><button className="btn btn-ghost" style={{width:'100%',justifyContent:'center'}}>{tr.importLeadsBtn}</button></a>
          </div>
        </div>
      </div>
    </div>
