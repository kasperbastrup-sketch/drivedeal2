...
export default function Analytics() {
  ...
  const [stats, setStats] = useState({
    totalSent: 0, totalOpened: 0, totalLeads: 0,
    coldLeads: 0, warmLeads: 0, sentLeads: 0,
    repliedLeads: 0, bookedLeads: 0, symbol: 'kr', avgCarPrice: 260000,
  })
  ...
  async function loadStats() {
    ...
    const symbol = symbols[dealer?.currency || 'DKK'] || 'kr'
    const avgPrice = dealer?.avg_car_price || 260000

    setStats({
      totalSent: logs.length,
      totalOpened: logs.filter((l:any) => l.opened).length,
      totalLeads: leads.length,
      coldLeads: leads.filter((l:any) => l.status === 'cold').length,
      warmLeads: leads.filter((l:any) => l.status === 'warm').length,
      sentLeads: leads.filter((l:any) => l.status === 'sent').length,
      repliedLeads: leads.filter((l:any) => l.status === 'replied').length,
      bookedLeads: leads.filter((l:any) => l.status === 'booked').length,
      symbol, avgCarPrice: avgPrice,
    })
    ...
    setMonthlyData(Object.entries(monthMap).slice(-6).map(([month,data]) => ({ month, ...data })))
    setLoading(false)
  }
  ...
  function fmtCurrency(n:number) {
    return stats.symbol === '€' || stats.symbol === '£' || stats.symbol === '$' ? `${stats.symbol}${fmt(n)}` : `${fmt(n)} ${stats.symbol}`
  }
    ...
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
        {[
          ...
          {label:'Est. omsætning',val:fmtCurrency(estimatedRevenue),color:'var(--gold)',sub:'Baseret på bookinger'},
        ].map(k => (
      ...
      {stats.totalSent === 0 && (
        <div className="panel" style={{textAlign:'center',padding:60}}>
          ...
          <div style={{fontSize:12,color:'var(--text3)'}}>Data vises her automatisk når systemet begynder at sende emails.</div>
        </div>
      )}
    </div>
  )
